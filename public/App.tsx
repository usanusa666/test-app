...
import { UploadIcon, SparklesIcon, TrashIcon, ExclamationTriangleIcon, PhotoIcon, BookOpenIcon, GlobeAltIcon } from './components/Icons';
...
export default function App() {
  const [view, setView] = useState<'studio' | 'recipes' | 'browser'>('studio');
  ...
  const commonButtonClass = "w-1/3 py-2 text-sm font-semibold flex items-center justify-center gap-2 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-base-300 focus:ring-brand-primary";
    ...
    <div className="min-h-screen bg-base-100 flex flex-col md:flex-row font-sans">
      ...
      <div className="w-full md:w-1/3 lg:w-1/4 bg-base-200 p-6 flex flex-col space-y-6 shadow-2xl">
        <header className="text-center">
          ...
          <p className="text-text-secondary mt-1">{view === 'studio' ? 'Image Studio' : view === 'recipes' ? 'Recipe Book' : 'Web Browser'}</p>
        </header>
        ...
        <div className="flex rounded-md bg-base-300 p-1">
          ...
          >
           ...
           <button 
            ...
            className={`${commonButtonClass} ${view === 'browser' ? activeButtonClass : inactiveButtonClass}`}
            aria-pressed={view === 'browser'}
          >
            <GlobeAltIcon className="w-4 h-4" /> Browser
          </button>
        </div>
        ...
        <div className="flex-grow flex flex-col space-y-6">
            {view === 'studio' ? (
                ...
                <div className="text-center text-text-secondary flex-grow flex flex-col items-center justify-center">
                 ...
                 <div className="text-center text-text-secondary flex-grow flex flex-col items-center justify-center">
                    <GlobeAltIcon className="w-16 h-16 text-base-300 mb-4" />
                    <p className="font-semibold">Web Browser</p>
                    <p className="text-sm">Navigate the web from right inside the app.</p>
                </div>
            )}
        </div>
